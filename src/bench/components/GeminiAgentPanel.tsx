...
interface GeminiAgentPanelProps {
...
export function GeminiAgentPanel({ onJointUpdate, cameraRefs, cameras }: GeminiAgentPanelProps) {
  const captureFrame = useCallback((): string | null => {
    // Use the first camera available (camera_1)
    const cameraId = cameras[0]?.id;
    if (!cameraId) {
      console.error('[GeminiAgent] No camera available');
      return null;
    }
    ...
    if (!handle || !camera) {
      console.error('[GeminiAgent] Camera handle not found:', cameraId);
      return null;
    }
    ...
    try {
      ...
      if (!pixels || pixels.length === 0) {
        console.error('[GeminiAgent] No pixels captured from camera');
        return null;
      }

      // Convert pixels to PNG data URL
      ...
      console.log('[GeminiAgent] Captured frame from', cameraId, width, 'x', height);
      return dataUrl;
    } catch (error) {
      console.error('[GeminiAgent] Frame capture failed:', error);
      return null;
    }
  }, [cameraRefs, cameras]);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <div>
          <div style={styles.headerTitle}>Gemini Agent</div>
          <div style={styles.headerSub}>Multimodal Robot Control</div>
        </div>
      </div>
      ...
      {lastAnalysis && (
        <div style={styles.analysisBox}>
          <div style={styles.analysisHeader}>Gemini Analysis</div>
